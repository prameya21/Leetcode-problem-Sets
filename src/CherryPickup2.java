public class CherryPickup2
{
    ...
    public int dfs(int[][] grid, int r, int c1, int c2, Integer[][][] memo)
    {
        if(r==grid.length)
            return 0;
        ...
        for(int i=-1;i<=1;i++)
        {
            for(int j=-1;j<=1;j++)
            {
                ...
                if(nc1<0 || nc1>=grid[0].length || nc2<0 || nc2>=grid[0].length)
                    continue;
                ans=Math.max(ans,dfs(grid,r+1,nc1,nc2,memo));
            }
        }
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println(obj.cherryPickup(new int[][]{{3,1,1},{2,5,1},{1,5,5},{2,1,1}}));
    }
}
