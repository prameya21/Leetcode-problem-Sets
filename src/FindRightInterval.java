...
public class FindRightInterval
{
    ...
    public int[] findRightInterval(int[][] intervals)
    {
        ...
        for(int i=0;i< intervals.length;i++)
        {
            ...
            int idx=binarySearch(sp,ep);
            if(idx<sp.length && sp[idx]>=ep)
                result[i]=map.get(sp[idx]);
            else
                result[i]=-1;
        }
        return result;
    ...
    public int binarySearch(int[] sp, int target)
    {
        int lo=0,hi=sp.length-1;
        while(lo<hi)
        {
            int mid=lo+(hi-lo)/2;
            if(sp[mid]>=target)
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println(Arrays.toString(obj.findRightInterval(new int[][]{{3,4},{2,3},{1,2}})));
    }
}
